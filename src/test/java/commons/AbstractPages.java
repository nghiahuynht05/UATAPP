...
public class AbstractPages {
    private static final Logger LOGGER = LogManager.getLogger( AbstractPages.class );
    ...
    public boolean MacthDataJSON(DataTable dataTable) {
        for (Map<Object, Object> data : dataTable.asMaps( String.class, String.class )) {
            try {
                ...
                expectData.put( "serviceNow", data.get( "serviceNow" ) );
                expectData.put( "addressPU", data.get( "addressPU" ) );
    ...
    public void getMenuService() throws JSONException {
    ...
    public void getContentMessage(String locator) {
        locator = String.format( locator, appPackageId );
        element = driver.findElement( By.id( locator ) );
        returnData = (element.getText()).replace( "\n", " " );
        LOGGER.info( "Actual response data: {} ", returnData );
    }
    ...
    public void sendKeyToElementById(String locator, String textValue) {
        if (textValue.equals( "randoms" )) {
            String code = "34";
            String phone;
            Random generator = new Random();
            phone = Integer.toString( generator.nextInt( 6 ) );

            locator = String.format( locator, appPackageId );
            element = driver.findElement( By.id( locator ) );
            element.clear();
            element.sendKeys( phone + code );
            phoneRanDoms = phone + code;
        }
        locator = String.format( locator, appPackageId );
        element = driver.findElement( By.id( locator ) );
        element.clear();
    ...
    public boolean checkMatchesMessage(String expectData) {
        LOGGER.info( "Actual response data: {} ", returnData );
        LOGGER.info( "Excepted data: {}", expectData );
